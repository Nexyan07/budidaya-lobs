...
<body class="bg-blue-50 text-gray-900">
    ...
    <div x-data="dashboard" x-init="init()" class="p-6 space-y-6">
        <!-- Header -->
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-xl sm:text-2xl font-bold text-blue-700 flex items-center gap-2">
                Dashboard IoT Lobster
            </h1>
            <div class="flex gap-6 items-center">
                
                <button @click="handleExport()"
                    class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded-lg flex items-center gap-2">
                    {{-- <i data-lucide="loading" class="w-4 h-4"></i> --}}
                    Refresh
                </button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
            <template x-for="sensor in sensors" :key="sensor.label">
                <div class="p-4 rounded-2xl shadow bg-white flex items-center gap-2 border-l-4"
                    ...
                    <i :data-lucide="sensor.icon" class="w-6 h-6 shrink-0"
                        :class="{
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div class="bg-white p-5 rounded-2xl shadow-md">
                ...
                <template x-for="device in devices" :key="device.label">
                    <div class="flex justify-between items-center px-3 py-2 border border-gray-300 rounded-xl mb-2">
        ...
        <div class="bg-white p-5 rounded-2xl shadow-md">
            ...
            <ul>
                <template x-for="alert in alerts" :key="alert.id">
                    <li class="border border-gray-300 rounded-xl p-3 mb-2 flex justify-between bg-blue-50">
            ...
            <div class="flex flex-col sm:flex-row justify-between gap-3 mb-4">
                ...
                <div class="flex gap-2">
                    <select x-model="filter"
                        class="max-sm:flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-400">
